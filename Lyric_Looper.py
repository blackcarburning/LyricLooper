...
ttk.Label(export_frame, text="⚠️ Install opencv-python, numpy, pillow",
foreground="red").pack()
else:
...
if not AUDIO_AVAILABLE:
ttk.Label(metro_frame, text="⚠️ Install pygame for metronome", foreground="orange").pack()
else:
...
self.duration_label.config(text=f"Duration: {loop_dur:.2f}s × ∞")
else:
total = loop_dur * self.loop_times.get()
self.duration_label.config(text=f"Duration: {total:.2f}s ({self.loop_times.get()}×)")
else:
...
if self.loop_infinite.get():
self.loop_status_label.config(text=f"Loop: {self.loop_current + 1} / ∞")
else:
